...
**✅ 单一职责原则（SRP）**
- 每个模块只有一个变化的理由
- API层负责解析前端HTTP请求，检查数据格式，调用Application层，把Application层的结果打包成HTTP响应发回前端
- Application层关注业务逻辑编排
- Domain层关注具体模块的内部算法逻辑
- Infrastructure层关注外部客户端服务实现
...
```
用户在前端页面发送请求，发送给了前端：
【index.html前端代码】，向后端发送HTTP POST请求，包括：
    - URL(例如/api/chat/stream)
    - 请求参数(包括session_id/user_id/message)
    ↓
【main.py】后端应用的入口是main.py，HTTP POST请求到达main.py
    - 中间件依次处理请求
    - FastAPI根据URL找到对应的路由处理器 chat_router
    ↓
【chat_router.py】
    - 提取参数：query, user_id, session_id
    - 在FastAPI中用 Depends() 实现依赖注入，以下为具体逻辑：
        检查 _legacy_streaming_service 是否存在
        - 如果不存在,创建并组装所有依赖:
        * llm_client (单例)
        * neo4j_query_service (单例)
        * message_repository (单例)
        * session_repository (单例)
        - 返回单例服务
    - 【API层调用Application层】的LegacyStreamingService来处理请求。
    【Application层会调用Domain层】处理核心内部逻辑，中间会涉及到对于Infrastructure层外部数据的访问，包括：
    阶段1: 路由判断
    ...
    阶段5: 流式输出
    SSE流返回给用户前端
        - StreamingResponse 逐块发送
        - 格式: data:{"content":"...", "message_type":2}
【返回给index.html前端】
    - 读取流式数据
    - 解析JSON
    - 实时追加到聊天窗口
